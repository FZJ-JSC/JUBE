...
Invocating ``jube result`` a second time will update the database specified by the ``file`` parameter. Without the ``primekey`` attribute of the ``key``-tag, three additional rows would be added to the ``results`` table which are identical to the already existing three rows. Setting the ``primekey`` attribute of the ``key``-tag to ``true`` adds the ``key`` to the ``primekeys``, ensuring that a new row is only added to the database table if the column values specified in the ``primekeys`` do not match exactly in the database table. In this example, no new rows would be added. The use of the ``database`` attribute ``primekeys`` is deprecated. Instead, use the ``primekey`` attribute of the ``key``. Updating ``primekeys`` is not supported.
